...
read_entities_funs = {
    "google-amazon": (read_rel_entities, read_rel_entities),
...
def read_entities(data_type: str, args: APrompt4EMArgs):
...
def process_ground_truth_few_shot(data, k=0.1, seed=2022, return_un_y=False):
    set_seed(seed)
    x_pos = []
    x_neg = []
    all_samples = []
    all_samples_y = []
    pair,label = data
    for i, line in enumerate(zip(pair,label)):
        pair,values =  line
        if int(values) == 1:
            x_pos.append(pair)
        else:
    ...
    x.extend(x_pos[:num_sample_pos])
    x.extend(x_neg[:num_sample_neg])
