...
def test_pseudo_labels(args: APrompt4EMArgs, data: PromptEMData, model):
...
def gen_pseudo_labels_by_confidence(args: APrompt4EMArgs, model, data_loader):
    model.eval()
    id = 0
    all_conf = []
    pos_all_conf = []
...
def gen_pseudo_labels_by_uncertainty(args: APrompt4EMArgs, model, data_loader):
    # MC Dropout
    model.eval()
...
def gen_pseudo_labels_by_fold_unfold(args: APrompt4EMArgs, model, labeled_dataloader, unlabeled_dataloader):
    """
...
def gen_pseudo_labels(args: APrompt4EMArgs, data: PromptEMData, model, prompt=True) -> int:
    """
