...
class PromptDataLoader(object):
    ...
    def tokenize(self) -> None:
        ...
        """
        for idx, wrapped_example in tqdm(enumerate(self.wrapped_dataset)):
            inputfeatures = InputFeatures(
                **self.tokenizer_wrapper.tokenize_one_example(wrapped_example, self.teacher_forcing),
                **wrapped_example[1]).to_tensor()
            guid = wrapped_example[1]['guid']
            res = self.tokenizer_wrapper.tokenizer([self.raw_dataset[guid].meta['text_a'],self.raw_dataset[guid].meta['text_b']], padding="max_length", truncation=True,max_length=self.args.maxlength)
            setattr(inputfeatures, f'left_q_input_ids', torch.tensor(res['input_ids'][0]))
            setattr(inputfeatures, f'right_q_input_ids', torch.tensor(res['input_ids'][1]))
            setattr(inputfeatures, f'left_attention_mask', torch.tensor(res['attention_mask'][0]))
            setattr(inputfeatures, f'right_attention_mask', torch.tensor(res['attention_mask'][1]))
            if self.args.pos_emd:
                left_emd =  self.raw_dataset[guid].meta['left']
                left_emd =  left_emd  +max(0,self.args.maxlength-len(left_emd))*[0]
                right_emd = self.raw_dataset[guid].meta['right']
                right_emd = right_emd  +max(0,self.args.maxlength-len(right_emd))*[0]
                setattr(inputfeatures, f'left_pos', torch.tensor(left_emd[:self.args.maxlength]))
                setattr(inputfeatures, f'right_pos', torch.tensor(right_emd[:self.args.maxlength]))
            '''
            for pos in ['left', 'right']:
                 
                q, k, att = [], [], []
                keys = ['title', 'brand', 'price', 'priceCurrency', 'description']
                for j, key in enumerate(keys):
                    if key == 'title':
                        q = self.tokenizer_wrapper.tokenizer.encode(self.raw_dataset[guid].meta[f'{pos}_{key}'],
                                                                      return_tensors="pt").squeeze()
                    else:
                        res = self.tokenizer_wrapper.tokenizer.encode(self.raw_dataset[guid].meta[f'{pos}_{key}'],
                                                                      return_tensors="pt").squeeze()
                        k.extend(res)
                        att.extend(len(res)*[j])
                setattr(inputfeatures, f'{pos}_q_input_ids', q)
                setattr(inputfeatures, f'{pos}_k_input_ids', torch.stack(k))
                setattr(inputfeatures, f'{pos}_att_input_ids',torch.tensor(att))
            '''
...
class PromptModel(nn.Module):
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        """
        batch,softemd = self.template.process_batch(batch)
...
class PromptForClassification(nn.Module):
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        """
        outputs,softemd = self.prompt_model(batch)
        outputs = self.verbalizer.gather_outputs(outputs)
        outputs_at_mask = self.extract_at_mask(outputs, batch)
        label_words_logits = self.verbalizer.process_outputs(outputs_at_mask, batch=batch)
        return label_words_logits,softemd
